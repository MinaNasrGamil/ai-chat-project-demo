...
class Router:
    ...
    def route(self, query: str, top_m=2, min_score=0.08, always_at_least_one=False):
        ...
        for sc, i in zip(scores[0], inds[0]):
            if i < 0:
                continue
            if sc >= min_score:
                results.append((self.doc_names[i], float(sc)))

        if always_at_least_one and not results and len(self.doc_names) > 0:
            best_i = int(inds[0][0])
            results = [(self.doc_names[best_i], float(scores[0][0]))]

        return results

    def explain_terms(self, query: str, top_k=5) -> List[str]:
